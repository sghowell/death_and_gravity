...
import pytest
from p8_vacuum_finite_contact_conversion import verify
...
KEYS = [
    ...
    "proof_checks",
    "exact_regulated_contact_and_bare_map",
    "full_amplitude_cancellation_and_owners",
    "actual_isolated_coordinate_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["exact_regulated_contact_and_bare_map"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 66
    assert d["checked_scalar_entries"] == 66
    assert len(d["proof_checks"]) == 23
    assert d["controls"]["rejected_inputs"] == 49
    assert "NOT_FULL_MS_MATCHING" in d["status"]
