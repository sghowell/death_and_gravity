...
import pytest
from p8_vacuum_affine_shear_response import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_Hamiltonian_and_metric_vertices",
    "actual_state_response_and_finite_amplitude",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["actual_state_response_and_finite_amplitude"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 43 and d["checked_scalar_entries"] == 348
    assert len(d["proof_checks"]) == 25
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 110
    assert (
        "NOT_CONTINUUM_RENORMALIZED_RESPONSE_SELF_ENERGY_INVERSE_QUANTUM_BACKGROUND"
        in d["status"]
    )
