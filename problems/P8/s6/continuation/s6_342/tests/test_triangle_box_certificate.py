"""Whole twenty-field native equality and immutable triangle/box-source contract."""

import copy
import json

import pytest
from p8_vacuum_affine_triangle_box_radiation import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_functional_labels_and_physical_radiation",
    "whole_uniform_remainder_interference_and_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_uniform_remainder_interference_and_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_triangle_box_radiation import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 22
    assert data["named_exact_check_count"] == len(audit.residuals()) == 337
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 337
    assert len(data["proof_checks"]) == len(audit.gates()) == 42
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 558
