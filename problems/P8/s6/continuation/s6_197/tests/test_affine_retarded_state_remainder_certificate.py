...
import pytest
from p8_vacuum_affine_retarded_state_remainder import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_retarded_memory_and_contact_remainder",
    "all_momentum_tail_and_reference_boundary",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["all_momentum_tail_and_reference_boundary"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 28 and d["checked_scalar_entries"] == 51
    assert len(d["proof_checks"]) == 38
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 119
    assert "NOT_SINGULAR_REFERENCE_COVARIANT_MATCHING_FULL_INVERSE" in d["status"]
