...
import pytest
from p8_vacuum_affine_local_spatial_hessian import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_spatial_polynomial_maps_and_fixed_local_Hessians",
    "proper_time_conversion_and_actual_CD_derivative_norm",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["proper_time_conversion_and_actual_CD_derivative_norm"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 37 and d["checked_scalar_entries"] == 177
    assert len(d["proof_checks"]) == 36
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 126
    assert "NOT_QUANTUM_SPATIAL_MATCHING_FULL_MIXED_INVERSE" in d["status"]
