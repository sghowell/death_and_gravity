"""Whole twenty-field equality and frozen factorized-bubble contract."""

import copy
import json

import pytest
from p8_vacuum_affine_factorized_bubble_radiation import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_factorized_source_kernel_loops_and_physical_branch",
    "whole_uniform_bounds_and_exact_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_uniform_bounds_and_exact_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_factorized_bubble_radiation import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 22
    assert data["named_exact_check_count"] == len(audit.residuals()) == 362
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 362
    assert len(data["proof_checks"]) == len(audit.gates()) == 55
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 403
