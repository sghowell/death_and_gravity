...
import pytest
from p8_vacuum_affine_temporal_tree_reorganization import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_finite_temporal_chart",
    "whole_recursive_tree_and_restricted_current_bounds",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_recursive_tree_and_restricted_current_bounds"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 101
