...
import pytest
from p8_vacuum_affine_two_real_collinear_current import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_offshell_current",
    "whole_collinear_factorization_and_uniform_bound",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_collinear_factorization_and_uniform_bound"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 82
