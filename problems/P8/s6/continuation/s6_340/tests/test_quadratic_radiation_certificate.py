"""Whole twenty-field equality and frozen quadratic-radiation contract."""

import copy
import json

import pytest
from p8_vacuum_affine_quadratic_radiation_cancellation import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_inverse_and_actual_mixed_loops",
    "whole_exact_quadratic_cancellation_and_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_exact_quadratic_cancellation_and_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_quadratic_radiation_cancellation import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 20
    assert data["named_exact_check_count"] == len(audit.residuals()) == 398
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 419
    assert len(data["proof_checks"]) == len(audit.gates()) == 39
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 441
