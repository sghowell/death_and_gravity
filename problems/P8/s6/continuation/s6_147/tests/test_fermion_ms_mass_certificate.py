...
import pytest
from p8_vacuum_fermion_ms_mass import verify
...
KEYS = [
    ...
    "proof_checks",
    "exact_massless_MS_anchors",
    "full_regulated_scalar_mass_transfer",
    "actual_primitive_mass_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["exact_massless_MS_anchors"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 49
    assert d["checked_scalar_entries"] == 49
    assert len(d["proof_checks"]) == 23
    assert d["controls"]["rejected_inputs"] == 51
    assert "NOT_OTHER_FORESTS" in d["status"]
