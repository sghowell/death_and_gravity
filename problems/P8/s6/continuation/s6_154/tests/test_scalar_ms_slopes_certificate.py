...
import pytest
from p8_vacuum_scalar_ms_slopes import verify
...
KEYS = [
    ...
    "proof_checks",
    "dimensional_local_sunset_and_complete_Cauchy_bounds",
    "proper_MS_conversion_and_integral_enclosures",
    "actual_finite_MS_slope_and_converted_OS_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["dimensional_local_sunset_and_complete_Cauchy_bounds"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert "NOT_FULL_GY14_FIELD_COUPLING_MATCHING" in d["status"]
