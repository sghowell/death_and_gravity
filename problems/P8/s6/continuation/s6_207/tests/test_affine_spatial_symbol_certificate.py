...
import pytest
from p8_vacuum_affine_spatial_symbol import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_two_time_polarization_factorization_and_normalized_symbol",
    "finite_UV_extraction_and_nonzero_massive_benchmark",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["finite_UV_extraction_and_nonzero_massive_benchmark"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 37 and d["checked_scalar_entries"] == 119
    assert len(d["proof_checks"]) == 39
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 129
    assert "NOT_FULL_CONTACT_REGULATOR_COVARIANT_MATCHING_ALL_P_NORM" in d["status"]
