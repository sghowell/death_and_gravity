"""Read-only report, exact manifests and every-field mutation rejection."""

import copy
import json

import pytest
from p8_vacuum_affine_matrix_adiabatic import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_matrix_frame_and_uniform_jets",
    "finite_reference_and_actual_state_tail",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["finite_reference_and_actual_state_tail"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 80 and d["checked_scalar_entries"] == 257
    assert len(d["proof_checks"]) == 53
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 112
    assert "NOT_RESPONSE_PARAMETER_REMAINDER_COVARIANT_SUBTRACTION" in d["status"]
