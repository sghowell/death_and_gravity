...
import pytest
from p8_vacuum_affine_reference_state_prefactor import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_initial_occupation_and_retarded_memory_correction",
    "full_contact_regulator_limit_and_curved_reference_decomposition",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_contact_regulator_limit_and_curved_reference_decomposition"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 28 and d["checked_scalar_entries"] == 135
    assert len(d["proof_checks"]) == 36
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 123
    assert (
        "NOT_JOINT_SPATIAL_ANALYTICITY_COVARIANT_MATCHING_FULL_INVERSE" in d["status"]
    )
