...
import pytest
from p8_vacuum_affine_tensor_noise_response import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_tensor_action_and_TT_class",
    "adjoint_energy_and_compatible_noise",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["adjoint_energy_and_compatible_noise"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 40 and d["checked_scalar_entries"] == 79
    assert len(d["proof_checks"]) == 32
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 109
    assert (
        "NOT_FULL_QUANTUM_TENSOR_STATE_SELF_ENERGY_REMAINDER_BACKGROUND" in d["status"]
    )
