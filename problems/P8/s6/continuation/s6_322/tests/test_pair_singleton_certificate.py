...
import pytest
from p8_vacuum_affine_pair_singleton_subtraction import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_pair_hierarchy",
    "whole_offshell_core_and_subtraction",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_offshell_core_and_subtraction"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    from p8_vacuum_affine_pair_singleton_subtraction import audit, hierarchy
    ...
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 75
    assert "NOT_FULL5116_INCLUSIVE_V_G_B_P8_CLOSURE" in data["status"]
