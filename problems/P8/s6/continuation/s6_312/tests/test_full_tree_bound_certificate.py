...
import pytest
from p8_vacuum_affine_uniform_two_real_tree_bound import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_regular_topology",
    "whole_common_gaps_and_complete_tree_bound",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_common_gaps_and_complete_tree_bound"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 110
