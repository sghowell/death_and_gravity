...
import pytest
from p8_vacuum_fermion_mixed_quartic import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_mixed_catalog",
    "joint_soft_remainder",
    "zero_soft_MS_reference_and_conversion",
    "actual_enclosure_and_frontier",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["complete_mixed_catalog"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 78
    assert d["checked_scalar_entries"] == 78
