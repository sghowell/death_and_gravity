...
import pytest
from p8_vacuum_affine_state_transport_soft_matching import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_state_continuity",
    "whole_signed_series_and_matched_reference",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_signed_series_and_matched_reference"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 131
