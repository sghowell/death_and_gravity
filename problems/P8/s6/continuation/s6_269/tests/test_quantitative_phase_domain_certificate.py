"""Immutable evaluated finite bounce phase-domain and volume comparison contract."""

import copy
import json

import pytest
from p8_vacuum_affine_quantitative_phase_domain import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_source_reference_and_evaluated_canonical_field_bounds",
    "whole_nonlinear_geometry_invariant_images_and_quantum_comparison",
...
def test_missing_and_extra_report_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_source_reference_and_evaluated_canonical_field_bounds"]
    else:
...
def test_exact_counts_and_original_matching_boundary():
    ...
    assert len(data["proof_checks"]) == 76
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 167
    assert (
        "NOT_TIME_EVOLUTION_ORDERING_MATCHING_REGULATOR_REMOVAL_OR_ORIGINAL_V_G_B_P8"
        in data["status"]
    )
    assert (
        data["prior_sha256"][
