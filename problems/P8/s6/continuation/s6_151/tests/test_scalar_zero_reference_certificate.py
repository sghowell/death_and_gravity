...
import pytest
from p8_vacuum_scalar_zero_reference import verify
...
KEYS = [
    ...
    "proof_checks",
    "regulated_sector_and_local_MS_anchors",
    "complete_wineglass_conversion_and_bounds",
    "actual_wineglass_MS_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["regulated_sector_and_local_MS_anchors"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["controls"]["rejected_inputs"] == 88
