...
import pytest
from p8_vacuum_affine_continuous_logarithmic_coefficient import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_continuous_kernels",
    "whole_aggregate_phase_and_uniform_measure_extension",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_aggregate_phase_and_uniform_measure_extension"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_continuous_logarithmic_coefficient import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 18
    assert data["named_exact_check_count"] == len(audit.residuals()) == 280
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 290
    assert len(data["proof_checks"]) == len(audit.gates()) == 43
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 90
    assert "NOT_QUANTUM_ALL_N_V_G_B_P8_CLOSURE" in data["status"]
