...
import pytest
from p8_vacuum_affine_full_spatial_remainder import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_constraint_vertices_pairs_and_original_scalar_shapes",
    "full_remainders_dimension_current_and_prepared_response",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_remainders_dimension_current_and_prepared_response"]
    else:
...
def test_exact_counts_and_full_parent_scope():
    ...
    assert d["named_exact_check_count"] == 73 and d["checked_scalar_entries"] == 457
    assert len(d["proof_checks"]) == 68
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 171
    assert "NOT_REDUCED_INVERSE_FULL_SOURCED_PARENT" in d["status"]
    assert (
        d["prior_sha256"][
            "same_actual_reference_parent_Hessian_not_full_nonlinear_source_free_family"
        ]
        is True
    )
