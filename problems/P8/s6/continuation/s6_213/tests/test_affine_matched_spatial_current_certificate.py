...
KEYS = [
    ...
    "proof_checks",
    "full_dimensional_limit_and_homogeneous_Hilbert_lift",
    "complete_matched_current_and_original_anchored_regulator",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["complete_matched_current_and_original_anchored_regulator"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 64 and d["checked_scalar_entries"] == 64
    assert len(d["proof_checks"]) == 52
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 135
    assert "NOT_FULL_SCALAR_MIXED_INVERSE_FINITE_COUPLING_BACKGROUND" in d["status"]
