...
import pytest
from p8_vacuum_flat_dirac_pressure import verify
...
KEYS = [
    ...
    "proof_checks",
    "projector_and_dimensional_pressure",
    "absolute_pressure_and_stress",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["absolute_pressure_and_stress"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["checked_scalar_entries"] == 37
    assert len(d["proof_checks"]) == 22
    assert len(d["controls"]) == 9
    assert d["controls"]["rejected_inputs"] == 93
    assert "NOT_CURVED_INTERACTING_PARENT" in d["status"]
