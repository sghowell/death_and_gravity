...
REPORT = (
    ROOT / "certificates/polynomial-vacuum-affine-massive-graviton-isolated-replay.json"
)
...
def source_files():
    return (
        sorted(
            ROOT.glob("src/p8_vacuum_affine_massive_graviton_cut_isolated_replay/*.py")
        )
...
REJECTED_SHA = "a45d6d297e8ecea8f6049983b69c8914125ef2fde89cea537fe578d34064d256"


def rejected_packet_evidence():
    """Pin the rejected evidence; never invoke or repair its order-dependent verifier."""
    archive = previous.ROOT.parents[4] / "research/p8-rejected-packets/s6_279"
    report = (
        archive / "certificates/polynomial-vacuum-affine-massive-graviton-pair-cut.json"
    )
    if sha(report) != REJECTED_SHA:
        raise ValueError("The immutable rejected S279 native evidence changed")
    data = json.loads(report.read_text())
    if (data["named_exact_check_count"], data["checked_scalar_entries"]) != (139, 409):
        raise ValueError("The archived S279 native rejection counts differ")
    if len(data["source_sha256"]) != 18:
        raise ValueError("The rejected S279 source manifest differs")
    if any(
        sha(archive / name) != digest for name, digest in data["source_sha256"].items()
    ):
        raise ValueError("An archived S279 scientific input changed")
    return {
        "status": "REJECTED_NOT_AN_ACCEPTED_PREDECESSOR",
        "raw_sha256": REJECTED_SHA,
        "unchanged_source_files": 18,
        "native_named_and_scalar": [139, 409],
        "cold_preflight_named_and_scalar": [130, 391],
    }
...
def prior_checks():
    ...
    return {
        ...
        "unpublished_S279_rejected_evidence": rejected_packet_evidence(),
        "explicit_parent_check_selection_invariant_under_warmup": True,
...
def build_report():
    ...
    return {
        "schema": 1,
        "claim": "P8-S6.280.ORIGINAL_MASSIVE_TWO_GRAVITON_CUT_FROM_LITERAL_EINSTEIN_AND_MATTER_VERTICES_WITH_COMPLETE_ANGULAR_SEWING_ALL_EVEN_SPIN_SQUARES_AND_PHYSICAL_WINDOW_BOUND",
        "date": "2026-09-15",
...
def main():
    ...
    if args.check:
        ...
        print(
            "P8 S6.280 original massive physical two-graviton cut replay passed; original P8 remains OPEN"
        )
    else:
