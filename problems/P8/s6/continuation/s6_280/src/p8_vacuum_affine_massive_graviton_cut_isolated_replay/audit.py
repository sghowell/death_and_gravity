...
MODEL = "original_massive_physical_two_graviton_order_invariant_replay_not_full_G"
OBSERVABLES = (
...
REJECTED_ITEM = {
    ...
    "status": "REJECTED_NATIVE_REPORT_CALL_ORDER_DEPENDENCE; SOURCES_AND_RAW_REPORT_ARCHIVED_UNCHANGED",
}

ITEM = {
    "id": "QG2_H8A444_original_massive_two_graviton_cut_from_literal_EH_and_matter_with_full_angular_sewing_all_even_spins_and_physical_window_bound",
...
def matching():
    return [*previous.matching(), dict(REJECTED_ITEM), dict(ITEM)]
...
def gates():
    return {
        **{
        ...
        "rejected_S279_is_explicitly_recorded_not_promoted": REJECTED_ITEM[
            "status"
        ].startswith("REJECTED_"),
        "all9_primitive_statuses_preserved": len(frontier()) == 9,
        "all134_previous_matching_records_preserved": matching()[:-2]
        == previous.matching()
