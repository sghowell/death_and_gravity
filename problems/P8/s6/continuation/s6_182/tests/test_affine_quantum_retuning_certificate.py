"""Read-only new-action report, explicit manifests and mutation rejection."""

import copy
import json

import pytest
from p8_vacuum_affine_quantum_retuning import verify
...
KEYS = [
    ...
    "proof_checks",
    "new_fixed_coefficient_and_stationary_mean",
    "quantitative_error_budget_and_actual_response",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["quantitative_error_budget_and_actual_response"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 63 and d["checked_scalar_entries"] == 74
    assert len(d["proof_checks"]) == 36
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 104
    assert "NOT_FULL_INTERACTING_QUANTUM_BACKGROUND" in d["status"]
