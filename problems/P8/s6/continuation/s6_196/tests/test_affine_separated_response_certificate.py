...
import pytest
from p8_vacuum_affine_separated_response import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_stress_vector_and_projection_tail",
    "ordered_support_continuum_weak_response",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["ordered_support_continuum_weak_response"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 32 and d["checked_scalar_entries"] == 81
    assert len(d["proof_checks"]) == 37
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 118
    assert "NOT_COINCIDENT_RETARDED_EXTENSION_OPERATOR_NORM_INVERSE" in d["status"]
