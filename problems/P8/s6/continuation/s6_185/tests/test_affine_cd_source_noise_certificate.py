...
import pytest
from p8_vacuum_affine_cd_source_noise import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_state_and_curved_covariance",
    "full_source_derivatives_and_force_conventions",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_source_derivatives_and_force_conventions"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 37 and d["checked_scalar_entries"] == 37
    assert len(d["proof_checks"]) == 43
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 107
    assert "NOT_METRIC_NOISE_QUANTUM_BACKGROUND" in d["status"]
