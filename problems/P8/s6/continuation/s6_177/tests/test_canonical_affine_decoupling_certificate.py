...
import pytest
from p8_vacuum_canonical_affine_decoupling import verify
...
KEYS = [
    ...
    "proof_checks",
    "fixed_canonical_family_and_gravitational_limit",
    "full_source_and_tree_observables",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_source_and_tree_observables"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 79
    assert d["checked_scalar_entries"] == 79
    assert len(d["proof_checks"]) == 19
    assert len(d["controls"]) == 9
    assert d["controls"]["rejected_inputs"] == 87
    assert "NOT_QUANTUM_CONTOUR_TRUNCATION_PHYSICAL_CUTOFF" in d["status"]
