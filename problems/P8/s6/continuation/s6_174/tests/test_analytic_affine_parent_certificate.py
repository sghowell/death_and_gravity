...
import pytest
from p8_vacuum_analytic_affine_parent import verify
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["actual_clock_vacuum_and_constraint_blocks"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["checked_scalar_entries"] == 844
    assert len(d["proof_checks"]) == 19
    assert len(d["controls"]) == 9
    assert d["controls"]["rejected_inputs"] == 84
    assert "NOT_QUANTUM_MATCHING_CUTOFF" in d["status"]
