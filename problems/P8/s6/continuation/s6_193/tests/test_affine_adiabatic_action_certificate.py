...
import pytest
from p8_vacuum_affine_adiabatic_action import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_local_action_and_covariant_matching",
    "dimension_limit_and_fixed_prescription",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["dimension_limit_and_fixed_prescription"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 172 and d["checked_scalar_entries"] == 172
    assert len(d["proof_checks"]) == 36
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 115
    assert "NOT_FULL_SPATIAL_MIXED_RESPONSE_INVERSE" in d["status"]
