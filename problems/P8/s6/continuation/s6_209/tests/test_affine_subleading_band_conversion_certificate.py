...
import pytest
from p8_vacuum_affine_subleading_band_conversion import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_original_shell_angular_shapes_and_full_massive_flat_benchmark",
    "actual_centered_cancellation_and_uniform_original_conversion_tail",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["actual_centered_cancellation_and_uniform_original_conversion_tail"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 76 and d["checked_scalar_entries"] == 181
    assert len(d["proof_checks"]) == 44
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 131
    assert (
        "NOT_FULL_CURVED_A1_EVALUATION_CONTACT_COVARIANT_MATCHING_RESPONSE_INVERSE"
        in d["status"]
    )
