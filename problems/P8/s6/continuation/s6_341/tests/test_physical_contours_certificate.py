"""Whole twenty-field native equality and immutable contour-source contract."""

import copy
import json

import pytest
from p8_vacuum_affine_physical_loop_contours import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_offshell_parameters_and_physical_contour",
    "whole_weighted_moments_TT_kernels_and_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_weighted_moments_TT_kernels_and_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_physical_loop_contours import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 22
    assert data["named_exact_check_count"] == len(audit.residuals()) == 403
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 439
    assert len(data["proof_checks"]) == len(audit.gates()) == 63
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 515
