...
import pytest
from p8_vacuum_flat_dirac_hadamard import verify
...
KEYS = [
    ...
    "proof_checks",
    "all_order_mass_symbol_bounds",
    "first_quantized_operator_norm_in_out_limit",
    "external_hadamard_theorem_application",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["first_quantized_operator_norm_in_out_limit"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["controls"]["rejected_inputs"] == 178
    assert "NOT_NUMERICAL_STRESS_INTERACTING_CURVED_STATE" in d["status"]
