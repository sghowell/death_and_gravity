...
import pytest
from p8_vacuum_fermion_vacuum import verify
...
KEYS = [
    ...
    "proof_checks",
    "exact_massless_vacuum_and_derivative_anchors",
    "actual_scalar_vacuum_and_assigned_forests",
    "actual_vacuum_primitive_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["exact_massless_vacuum_and_derivative_anchors"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 53
    assert d["checked_scalar_entries"] == 53
    assert len(d["proof_checks"]) == 23
    assert d["controls"]["rejected_inputs"] == 91
    assert "NOT_FULL_VACUUM_MATCHING" in d["status"]
