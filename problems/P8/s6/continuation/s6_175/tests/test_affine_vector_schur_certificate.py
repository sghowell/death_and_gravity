...
import pytest
from p8_vacuum_affine_vector_schur import verify
...
KEYS = [
    ...
    "proof_checks",
    "exact_sourced_vector_and_homogeneous_reduction",
    "full_source_and_finite_germ_estimates",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_source_and_finite_germ_estimates"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["checked_scalar_entries"] == 213
    assert len(d["proof_checks"]) == 19
    assert len(d["controls"]) == 9
    assert d["controls"]["rejected_inputs"] == 114
    assert "NOT_FULL_SOURCE_REAL_TIME_QUANTUM_MATCHING_CUTOFF" in d["status"]
