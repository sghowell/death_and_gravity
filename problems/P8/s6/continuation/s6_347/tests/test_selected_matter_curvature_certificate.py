"""Every native-report field and immutable source contract is checked."""

import copy
import json

import pytest
from p8_vacuum_affine_selected_matter_curvature_coefficient import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_complete_inventory_local_and_mixed_matching",
    "whole_exact_extra_and_total_known_coefficient",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_exact_extra_and_total_known_coefficient"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_selected_matter_curvature_coefficient import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 21
    assert data["named_exact_check_count"] == len(audit.residuals()) == 378
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 397
    assert len(data["proof_checks"]) == len(audit.gates()) == 45
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 859
