"""Complete twenty-field report equality and immutable source contract."""

import copy
import json

import pytest
from p8_vacuum_affine_box_curvature_coefficient import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_flat_lift_and_generic_box_matching",
    "whole_finite_moment_and_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_finite_moment_and_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_box_curvature_coefficient import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 21
    assert data["named_exact_check_count"] == len(audit.residuals()) == 753
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 3206
    assert len(data["proof_checks"]) == len(audit.gates()) == 43
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 727
