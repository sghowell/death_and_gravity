...
import pytest
from p8_vacuum_affine_curved_linear_conversion import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_complete_WKB_and_curved_second_grade_source_densities",
    "full_tensor_curved_conversion_operator_and_coefficient_norms",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_tensor_curved_conversion_operator_and_coefficient_norms"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 49 and d["checked_scalar_entries"] == 92
    assert len(d["proof_checks"]) == 40
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 132
    assert (
        "NOT_FULL_ONE_BALL_UV_CONTACT_COVARIANT_MATCHING_RESPONSE_INVERSE"
        in d["status"]
    )
