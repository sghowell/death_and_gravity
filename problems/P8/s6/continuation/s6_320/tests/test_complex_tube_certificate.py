...
import pytest
from p8_vacuum_affine_relative_energy_complex_tube import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_complex_geometry",
    "whole_complex_continuation_and_Cauchy_majorants",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_complex_continuation_and_Cauchy_majorants"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 108
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 73
    assert "NOT_SOFT_FACES_OR_FULL_V_G_B_P8_CLOSURE" in data["status"]
