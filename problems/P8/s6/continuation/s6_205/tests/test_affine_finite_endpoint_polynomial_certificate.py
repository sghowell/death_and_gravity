...
import pytest
from p8_vacuum_affine_finite_endpoint_polynomial import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_endpoint_degree_partition_and_finite_radial_bounds",
    "original_two_leg_tail_and_restored_actual_finite_piece",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["original_two_leg_tail_and_restored_actual_finite_piece"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 29 and d["checked_scalar_entries"] == 37
    assert len(d["proof_checks"]) == 39
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 127
    assert "NOT_REMAINING_UV_CONTACT_MATCHING_FULL_RESPONSE_INVERSE" in d["status"]
