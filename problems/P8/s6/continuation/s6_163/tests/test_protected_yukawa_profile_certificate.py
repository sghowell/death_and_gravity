...
import pytest
from p8_vacuum_protected_yukawa_profile import verify
...
KEYS = [
    ...
    "proof_checks",
    "new_profile_and_dimensional_loop_grade",
    "actual_mass_floor_and_conditional_clock_screen",
    "identical_named_coefficients_and_classical_match",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["actual_mass_floor_and_conditional_clock_screen"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 52
    assert d["checked_scalar_entries"] == 52
    assert len(d["proof_checks"]) == 29
    assert d["controls"]["rejected_inputs"] == 224
    assert "NOT_ROLLING_STATE_PHYSICAL_REMAINDER" in d["status"]
