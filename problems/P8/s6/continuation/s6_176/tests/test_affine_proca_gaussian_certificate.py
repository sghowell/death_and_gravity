...
import pytest
from p8_vacuum_affine_proca_gaussian import verify
...
KEYS = [
    ...
    "proof_checks",
    "conditional_Gaussian_state_and_canonical_bridge",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["specified_vector_stress_and_reference_clock_response"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 64
    assert d["checked_scalar_entries"] == 94
    assert len(d["proof_checks"]) == 19
    assert len(d["controls"]) == 9
    assert d["controls"]["rejected_inputs"] == 100
    assert "NOT_FULL_PARENT_QUANTUM_SOLUTION_MATCHING_CUTOFF" in d["status"]
