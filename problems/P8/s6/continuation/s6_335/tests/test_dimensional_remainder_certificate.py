...
import pytest
from p8_vacuum_affine_dimensional_real_remainder import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_same_D_Born_and_soft_remainder",
    "whole_integrated_bounds_and_original_recoil_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_integrated_bounds_and_original_recoil_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_dimensional_real_remainder import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 19
    assert data["named_exact_check_count"] == len(audit.residuals()) == 301
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 301
    assert len(data["proof_checks"]) == len(audit.gates()) == 158
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 251
    assert "NOT_VIRTUAL_HARD_V_G_B_P8_CLOSURE" in data["status"]
