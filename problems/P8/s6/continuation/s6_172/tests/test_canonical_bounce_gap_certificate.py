...
import pytest
from p8_vacuum_canonical_bounce_gap import verify
...
KEYS = [
    ...
    "proof_checks",
    "correlated_free_null_stress",
    "minimal_parent_equation_and_map",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["minimal_parent_equation_and_map"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 35
    assert d["checked_scalar_entries"] == 35
    assert len(d["proof_checks"]) == 22
    assert len(d["controls"]) == 9
    assert d["controls"]["rejected_inputs"] == 52
    assert "NOT_FULL_INTERACTING_PARENT_DHOST_ROW" in d["status"]
