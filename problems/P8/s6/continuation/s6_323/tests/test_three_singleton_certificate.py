...
import pytest
from p8_vacuum_affine_three_singleton_subtraction import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_connected_lines",
    "whole_forest_grouping_and_complete_subtraction",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_forest_grouping_and_complete_subtraction"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    from p8_vacuum_affine_three_singleton_subtraction import audit, forest

    assert data["named_exact_check_count"] == len(audit.residuals())
    assert data["checked_scalar_entries"] == audit.scalar_entry_count()
    assert len(data["proof_checks"]) == len(audit.gates())
    assert len(forest.data()["whole_soft_canonical_power_stripped_parts"]) == 8
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 79
    assert "NOT_INCLUSIVE_ALL_N_V_G_B_P8_CLOSURE" in data["status"]
