...
import pytest
from p8_vacuum_two_loop_elastic_cut import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_physical_one_loop_amplitude_bounds",
    "complete_two_loop_elastic_cut_and_ownership",
    "actual_complete_two_loop_cut_and_improved_band",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["complete_two_loop_elastic_cut_and_ownership"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["controls"]["rejected_inputs"] == 234
    assert "NOT_PHYSICAL_TRUNCATION_GLOBAL_DISPERSION" in d["status"]
