...
import pytest
from p8_vacuum_fermion_outer_ms import verify
...
KEYS = [
    ...
    "proof_checks",
    "finite_outer_MS_reference",
    "dimensional_OS_spectral_representation",
    "local_parent_reference_conversion",
    "actual_converted_family_frontier",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["dimensional_OS_spectral_representation"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["checked_scalar_entries"] == 54
    assert len(d["proof_checks"]) == 28
    assert d["controls"]["rejected_inputs"] == 53
