...
import pytest
from p8_vacuum_affine_flat_spatial_conversion import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_tensor_and_exact_subtraction_conversion",
    "retarded_origin_and_uniform_spatial_bounds",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["retarded_origin_and_uniform_spatial_bounds"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 34 and d["checked_scalar_entries"] == 994
    assert len(d["proof_checks"]) == 35
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 122
    assert (
        "NOT_CURVED_SPATIAL_MATCHING_PHYSICAL_LOCAL_POLYNOMIAL_FULL_INVERSE"
        in d["status"]
    )
