...
import pytest
from p8_vacuum_affine_three_real_probability_overlap import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_external_residues",
    "whole_proper_faces_and_probability_transfer",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_proper_faces_and_probability_transfer"]
    else:
...
def test_exact_counts_and_historical_scope():
    from p8_vacuum_affine_three_real_probability_overlap import audit, projection

    data = verify.build_report()
    assert len(data["source_sha256"]) == 20
    assert data["named_exact_check_count"] == len(audit.residuals()) == 313
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 667
    assert len(data["proof_checks"]) == len(audit.gates()) == 80
    assert projection.data()["whole_term_inventory"] == {
        "all": 27,
        "contains_full": 15,
        "proper": 12,
    }
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 81
