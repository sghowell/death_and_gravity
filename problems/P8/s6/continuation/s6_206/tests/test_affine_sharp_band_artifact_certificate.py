...
import pytest
from p8_vacuum_affine_sharp_band_artifact import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_full_polarization_symbol_and_angular_coefficient",
    "original_sharp_geometry_and_actual_asymptotic_conversion",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["original_sharp_geometry_and_actual_asymptotic_conversion"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 34 and d["checked_scalar_entries"] == 272
    assert len(d["proof_checks"]) == 38
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 128
    assert "NOT_FULL_REGULATOR_COVARIANT_MATCHING_PHYSICAL_DIVERGENCE" in d["status"]
