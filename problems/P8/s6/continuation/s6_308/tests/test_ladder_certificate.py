...
import pytest
from p8_vacuum_affine_leading_ladder_coulomb_phase import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_full_D_ladders",
    "whole_exact_phase_and_uniform_Gamma_bound",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_exact_phase_and_uniform_Gamma_bound"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 60
