...
import pytest
from p8_vacuum_affine_flat_scalar_quotient_inverse import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_covariant_projectors_ordered_gauge_and_reference_factorization",
    "complete_causal_matrix_inverse_uniform_spaces_and_physical_units",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["complete_causal_matrix_inverse_uniform_spaces_and_physical_units"]
    else:
...
def test_exact_counts_and_uncompleted_full_coupled_boundary():
    ...
    assert (
        data["named_exact_check_count"] == 44 and data["checked_scalar_entries"] == 105
    )
    assert len(data["proof_checks"]) == 25
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 180
    assert "NOT_FULL_CURVED_COUPLED_INVERSE" in data["status"]
    assert (
        data["prior_sha256"][
