...
import pytest
from p8_vacuum_affine_prepared_ward_reconstruction import verify
...
KEYS = [
    ...
    "proof_checks",
    "prepared_synchronous_maps_ordered_Ward_and_chart_terms",
    "three_scalar_reduction_and_explicit_known_piece_norms",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["three_scalar_reduction_and_explicit_known_piece_norms"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 38 and d["checked_scalar_entries"] == 100
    assert len(d["proof_checks"]) == 38
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 147
    assert "NOT_MISSING_SCALAR_MATCHING_FULL_RESPONSE_REDUCED_INVERSE" in d["status"]
