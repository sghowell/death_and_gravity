"""Immutable explicit finite local quantum regulator contract."""

import copy
import json

import pytest
from p8_vacuum_affine_local_quantum_regulator import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_source_time_connection_full_ordering_and_physical_contacts",
    "whole_phase_localization_and_convergent_quantum_dynamics",
...
def test_missing_and_extra_report_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_source_time_connection_full_ordering_and_physical_contacts"]
    else:
...
def test_exact_counts_and_original_matching_boundary():
    ...
    assert len(data["proof_checks"]) == 33
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 141
    assert (
        "NOT_EVALUATED_PHYSICAL_LEAKAGE_ORDERING_MATCHING_REGULATOR_REMOVAL_OR_ORIGINAL_V_G_B_P8"
        in data["status"]
    )
    assert (
        data["prior_sha256"][
