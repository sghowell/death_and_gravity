...
import pytest
from p8_vacuum_affine_covariant_current import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_metric_and_local_Euler_bounds",
    "full_current_and_canonical_response",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_current_and_canonical_response"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 22 and d["checked_scalar_entries"] == 22
    assert len(d["proof_checks"]) == 38
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 116
