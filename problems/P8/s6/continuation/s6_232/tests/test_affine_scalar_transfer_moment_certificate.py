"""Immutable transfer spectral-moment report and every-field mutation rejection."""

import copy
import json

import pytest
from p8_vacuum_affine_scalar_transfer_moment import verify
...
KEYS = [
    ...
    "proof_checks",
    "original_crossing_transfer_data_and_first_elastic_derivative",
    "mandatory_full_moment_first_elastic_bound_and_separate_positive_atom",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["mandatory_full_moment_first_elastic_bound_and_separate_positive_atom"]
    else:
...
def test_exact_counts_and_conditional_boundary():
    ...
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 225
    assert (
        "CONDITIONAL_FULL_SPECTRAL_WEIGHT_AND_HIGHER_COEFFICIENT_REQUIREMENT"
        in data["status"]
    )
    assert "NOT_FULL_LOOP_BOUND_PHYSICAL_CUTOFF" in data["status"]
    assert (
        data["prior_sha256"][
