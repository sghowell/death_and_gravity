...
import pytest
from p8_vacuum_affine_uniform_soft_current_bound import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_weighted_geometry",
    "whole_null_grading_and_all_finite_current_majorant",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_null_grading_and_all_finite_current_majorant"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 96
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 73
    assert "NOT_INCLUSIVE_RATE_OR_FULL_V_G_B_P8_CLOSURE" in data["status"]
