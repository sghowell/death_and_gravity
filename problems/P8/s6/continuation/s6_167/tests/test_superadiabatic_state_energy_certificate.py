...
import pytest
from p8_vacuum_superadiabatic_state_energy import verify
...
KEYS = [
    ...
    "proof_checks",
    "analytic_complex_disk_and_exact_frames",
    "complete_transition_and_energy_enclosures",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["complete_transition_and_energy_enclosures"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["controls"]["rejected_inputs"] == 169
    assert "NOT_ABSOLUTE_STRESS_INTERACTING_CURVED_STATE" in d["status"]
