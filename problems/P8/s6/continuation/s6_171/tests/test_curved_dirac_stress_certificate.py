...
import pytest
from p8_vacuum_curved_dirac_stress import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_projector_and_covariant_finite_parts",
    "absolute_curved_stress",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["absolute_curved_stress"]
    else:
        changed["full_parent_bounce_controlled"] = True
    with pytest.raises(ValueError, match="differs"):
        verify.validate_report(changed, d)
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 58
    assert d["checked_scalar_entries"] == 58
    assert len(d["proof_checks"]) == 32
    assert len(d["controls"]) == 9
    assert d["controls"]["rejected_inputs"] == 185
    assert "NOT_FULL_INTERACTING_PARENT_BACKGROUND_RESPONSE" in d["status"]
