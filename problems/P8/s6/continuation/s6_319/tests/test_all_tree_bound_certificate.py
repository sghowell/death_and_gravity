...
import pytest
from p8_vacuum_affine_uniform_all_tree_bound import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_hard_core",
    "whole_vertex_energy_and_complete_tree_majorant",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_vertex_energy_and_complete_tree_majorant"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 120
