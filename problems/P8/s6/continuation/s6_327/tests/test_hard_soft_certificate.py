...
import pytest
from p8_vacuum_affine_known_hard_soft_subtraction import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_selected_interference",
    "whole_uniform_signed_soft_integrals",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_uniform_signed_soft_integrals"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_known_hard_soft_subtraction import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 17
    assert data["named_exact_check_count"] == len(audit.residuals()) == 268
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 268
    assert len(data["proof_checks"]) == len(audit.gates()) == 35
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 87
