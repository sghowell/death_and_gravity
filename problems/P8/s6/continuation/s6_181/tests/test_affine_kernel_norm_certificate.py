"""Frozen report equality, explicit manifest and field mutation rejection."""

import copy
import json

import pytest
from p8_vacuum_affine_kernel_norm import verify
...
KEYS = [
    ...
    "proof_checks",
    "continuous_density_and_band_bounds",
    "same_kernel_norm_and_current_inverse_transfer",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["same_kernel_norm_and_current_inverse_transfer"]
    else:
        bad["full_inverse_and_quantum_background_complete"] = True
    with pytest.raises(ValueError, match="differs"):
        verify.validate_report(bad, d)
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 83
    assert d["checked_scalar_entries"] == 83
