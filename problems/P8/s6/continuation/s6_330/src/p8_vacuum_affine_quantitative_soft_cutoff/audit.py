"""Positive-energy coefficient and quantitative leading cutoff; original P8 open."""

from functools import cache

import sympy as s
from p8_vacuum_affine_complement_measure.split import clean
from p8_vacuum_affine_leading_cloud_logarithmic_coefficient import audit as previous

from . import conditioning, increment, source

MODEL = "original_positive_energy_log_and_quantitative_cutoff_not_full_P8"
OBSERVABLES = (
    "whole_original_source",
    "whole_positive_energy_coefficient_increment",
    "whole_quantitative_conditioned_cutoff",
)

ITEM = {
    "id": "QG2_H8A494_positive_energy_Lipschitz_coefficient_and_quantitative_IR_cutoff",
    "status": "POSITIVE_ENERGY_LIPSCHITZ_AND_QUANTITATIVE_LEADING_CUTOFF_NOT_INTERACTING_P8",
}
...
def require_sector(value):
    if not isinstance(value, str) or value not in (
        "positive_energy_increment_at_fixed_E_u",
        "conditioned_missing_energy",
        "quantitative_D4_marked_cutoff",
    ):
...
def packets():
    return {
        ...
        "whole_positive_energy_coefficient_increment": increment.data(),
        "whole_quantitative_conditioned_cutoff": conditioning.data(),
    }
...
def gates():
    return {
        **{
        ...
        "all185_previous_matching_records_preserved": matching()[:-1]
        == previous.matching()
        and len(previous.matching()) == 185,
...
def observable():
    return {
        "established": "For sigma=tau+rho positive at fixed original E,u and total energy<=1/8, the complete logarithmic coefficient changes by at most40000*massrho in uniform angular TT norm. Exact missing-energy moments and a relative calorimetric normalization bound give physical finite-cutoff versus limiting conditioned mean error<70000*eta/kappa^(5/2) and second-moment error<2000000000*x*eta/kappa^4, uniformly0<eta<=x<=1/8.",
        "domain": "Original action, parameters and recoil; fixed Born D4 leading reference and index0<=a<=1 (the original a<4/(5kappa)). The full complex coefficient and total-energy conditioning are kept, including extremely small unexpanded normalization.",
        "historical_qualification": qualifications(),
        "not_established": "Arbitrary signed angular-energy perturbations, changing hard E,u, state-dependent emission dynamics, marked dimensional/evanescent conversion, finite radiative hard matching or common-regulator detector rate, interacting state or all-N hard amplitudes, quantum unitarity, absolute Regge, common-parent bounce or original V/G/B/P8 closure.",
    }
...
def controls():
    return {
        "original_positive_increment_recoil": True,
        "full_complex_coefficient_and_every_pair_phase": True,
        "positive_measure_extension_not_arbitrary_signed": True,
        "one_and_two_distinguished_emissions_kept": True,
        "relative_rare_cut_normalization_not_dropped": True,
        "same_Born_mark_subtracted_before_mixture": True,
        "D4_leading_only_not_hard_dimensional_interacting_completion": True,
