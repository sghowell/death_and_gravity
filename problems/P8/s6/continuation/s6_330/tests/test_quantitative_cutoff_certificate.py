...
import pytest
from p8_vacuum_affine_quantitative_soft_cutoff import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_positive_energy_increment",
    "whole_quantitative_conditioned_infrared_cutoff",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_quantitative_conditioned_infrared_cutoff"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_quantitative_soft_cutoff import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 17
    assert data["named_exact_check_count"] == len(audit.residuals()) == 255
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 255
    assert len(data["proof_checks"]) == len(audit.gates()) == 40
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 96
