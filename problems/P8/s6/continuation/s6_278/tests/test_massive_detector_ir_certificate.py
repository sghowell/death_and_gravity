...
import pytest
from p8_vacuum_affine_massive_detector_ir_cut import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_massive_vacuum_and_massless_pair_cut",
    "whole_soft_resolution_and_crossing_subtraction",
...
def test_missing_and_extra_fields_are_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_soft_resolution_and_crossing_subtraction"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert (data["named_exact_check_count"], data["checked_scalar_entries"]) == (72, 81)
