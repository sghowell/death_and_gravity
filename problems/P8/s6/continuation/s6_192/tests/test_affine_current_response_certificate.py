...
import pytest
from p8_vacuum_affine_current_response import verify
...
KEYS = [
    ...
    "proof_checks",
    "physical_metric_vertices_and_exact_finite_band",
    "ordered_adiabatic_comparison_and_complete_C2_current",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["ordered_adiabatic_comparison_and_complete_C2_current"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 62 and d["checked_scalar_entries"] == 641
    assert len(d["proof_checks"]) == 41
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 114
    assert "NOT_FIXED_COVARIANT_MATCHING_FULL_RESPONSE_INVERSE" in d["status"]
