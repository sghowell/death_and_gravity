...
import pytest
from p8_vacuum_affine_weighted_channel_resolvent import verify
...
KEYS = [
    ...
    "proof_checks",
    "original_full_complex_channel_coercivity",
    "weighted_channel_corrections_curvature_composition_and_units",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["weighted_channel_corrections_curvature_composition_and_units"]
    else:
...
def test_exact_counts_and_uncompleted_actual_quantum_boundary():
    ...
    assert (
        data["named_exact_check_count"] == 35 and data["checked_scalar_entries"] == 35
    )
    assert len(data["proof_checks"]) == 25
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 183
    assert "NOT_ACTUAL_FULL_QUANTUM_INVERSE" in data["status"]
    assert (
        data["prior_sha256"][
