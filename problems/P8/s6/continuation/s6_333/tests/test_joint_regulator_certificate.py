...
import pytest
from p8_vacuum_affine_joint_soft_regulator import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_kernels_and_moments",
    "whole_quantitative_joint_regulator_bounds",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_quantitative_joint_regulator_bounds"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_joint_soft_regulator import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 19
    assert data["named_exact_check_count"] == len(audit.residuals()) == 274
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 274
    assert len(data["proof_checks"]) == len(audit.gates()) == 41
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 175
