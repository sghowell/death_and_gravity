...
import pytest
from p8_vacuum_full_phi_normalization import verify
...
KEYS = [
    ...
    "proof_checks",
    "regulator_complete_field_and_scalar_parameter_map",
    "complete_quadratic_ownership_and_OS_extension",
    "actual_complete_Phi_two_loop_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["regulator_complete_field_and_scalar_parameter_map"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["controls"]["rejected_inputs"] == 96
    assert "NOT_FULL_FOUR_POINT_VACUUM_SOURCE" in d["status"]
