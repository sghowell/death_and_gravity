...
import pytest
from p8_vacuum_clock_transparent_map import verify
...
KEYS = [
    ...
    "proof_checks",
    "polynomial_gate_and_two_domain_grading",
    "common_class_full_resolvent_action_comparison",
    "named_vacuum_and_clock_argument_dictionary",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["common_class_full_resolvent_action_comparison"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 61
    assert d["checked_scalar_entries"] == 61
    assert len(d["proof_checks"]) == 25
    assert d["controls"]["rejected_inputs"] == 180
    assert "NOT_BACKGROUND_SOLUTION_QUANTUM_STATE_PHYSICAL_REMAINDER" in d["status"]
