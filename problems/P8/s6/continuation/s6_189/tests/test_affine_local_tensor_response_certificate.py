...
import pytest
from p8_vacuum_affine_local_tensor_response import verify
...
KEYS = [
    ...
    "proof_checks",
    "direct_curvature_and_fixed_prescription",
    "full_spatial_operator_and_Sobolev_bound",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["full_spatial_operator_and_Sobolev_bound"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 111
    assert (
        "NOT_COMPLETE_DETERMINANT_RESPONSE_INVERSE_FINITE_COUPLING_BACKGROUND"
        in d["status"]
    )
