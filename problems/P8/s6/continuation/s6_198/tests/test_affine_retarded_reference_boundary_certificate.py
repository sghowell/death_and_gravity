...
import pytest
from p8_vacuum_affine_retarded_reference_boundary import verify
...
KEYS = [
    ...
    "proof_checks",
    "exact_retarded_identity_and_Cauchy_coefficients",
    "finite_reference_and_full_remaining_sector",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["finite_reference_and_full_remaining_sector"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 57 and d["checked_scalar_entries"] == 57
    assert len(d["proof_checks"]) == 39
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 120
    assert "NOT_CONTACT_ENDPOINT_COVARIANT_SPATIAL_MATCHING_FULL_INVERSE" in d["status"]
