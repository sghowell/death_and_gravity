...
import pytest
from p8_vacuum_affine_dimensional_gravity_radiation import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_dimensionally_complete_tree",
    "whole_canonical_47_graph_tensor_and_polarization_sew",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_canonical_47_graph_tensor_and_polarization_sew"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_dimensional_gravity_radiation import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 19
    assert data["named_exact_check_count"] == len(audit.residuals()) == 295
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 295
    assert len(data["proof_checks"]) == len(audit.gates()) == 32
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 213
    assert "NOT_FINITE_HARD_V_G_B_P8_CLOSURE" in data["status"]
