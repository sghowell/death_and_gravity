...
import pytest
from p8_vacuum_fermion_quadratic_forests import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_proper_cycle_catalog",
    "paired_MS_forest_and_regions",
    "on_shell_nonlocal_projection",
    "actual_enclosure_and_partial_frontier",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["complete_proper_cycle_catalog"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 68
    assert d["checked_scalar_entries"] == 74
    assert len(d["proof_checks"]) == 24
    assert d["controls"]["rejected_inputs"] == 64
    assert "NOT_FINITE_MS_LOCAL_REFERENCES" in d["status"]
