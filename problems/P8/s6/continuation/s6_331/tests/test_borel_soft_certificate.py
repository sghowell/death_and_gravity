...
import pytest
from p8_vacuum_affine_borel_soft_conversion import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_Borel_soft_conversion",
    "whole_conditioned_cloud_transfer_and_cutoff",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_conditioned_cloud_transfer_and_cutoff"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_borel_soft_conversion import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 20
    assert data["named_exact_check_count"] == len(audit.residuals()) == 257
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 272
    assert len(data["proof_checks"]) == len(audit.gates()) == 51
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 99
