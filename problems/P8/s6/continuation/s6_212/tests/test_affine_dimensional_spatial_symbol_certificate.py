...
import pytest
from p8_vacuum_affine_dimensional_spatial_symbol import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_dimensional_geometry_modes_and_fixed_source_UV_coefficients",
    "continued_fixed_counterterm_original_MSbar_finite_part_and_norms",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["continued_fixed_counterterm_original_MSbar_finite_part_and_norms"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 73 and d["checked_scalar_entries"] == 379
    assert len(d["proof_checks"]) == 41
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 134
    assert "NOT_FULL_DIMENSION_LIMIT_ASSEMBLED_RESPONSE_INVERSE" in d["status"]
