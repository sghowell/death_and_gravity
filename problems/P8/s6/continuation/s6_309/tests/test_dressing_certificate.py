...
import pytest
from p8_vacuum_affine_single_residual_soft_dressing import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_signed_soft_series",
    "whole_marked_operator_and_weighted_variation_bound",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_marked_operator_and_weighted_variation_bound"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 63
