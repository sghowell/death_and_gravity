...
import pytest
from p8_vacuum_affine_radiative_logarithmic_soft_coefficient import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_logarithmic_conventions",
    "whole_radiative_coefficient_bounds_and_named_measure",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_radiative_coefficient_bounds_and_named_measure"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_radiative_logarithmic_soft_coefficient import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 21
    assert data["named_exact_check_count"] == len(audit.residuals()) == 400
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 433
    assert len(data["proof_checks"]) == len(audit.gates()) == 64
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 85
    assert "NOT_FULL_LOOP_ALL_N_V_G_B_P8_CLOSURE" in data["status"]
