...
import pytest
from p8_vacuum_affine_local_tadpole_radiation import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_D_contractions_and_radiation",
    "whole_uniform_bounds_and_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_uniform_bounds_and_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_local_tadpole_radiation import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 20
    assert data["named_exact_check_count"] == len(audit.residuals()) == 340
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 376
    assert len(data["proof_checks"]) == len(audit.gates()) == 61
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 327
