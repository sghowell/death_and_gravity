...
import pytest
from p8_vacuum_full_two_loop_amplitude import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_four_point_ownership_and_coordinate_matching",
    "formal_two_loop_assembly_and_error_rule",
    "actual_complete_canonical_Phi_amplitude_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["complete_four_point_ownership_and_coordinate_matching"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 41
    assert d["checked_scalar_entries"] == 41
    assert len(d["proof_checks"]) == 25
    assert d["controls"]["rejected_inputs"] == 90
    assert "NOT_VACUUM_SOURCE_PHYSICAL_TRUNCATION" in d["status"]
