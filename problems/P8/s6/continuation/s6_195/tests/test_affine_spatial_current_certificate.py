...
import pytest
from p8_vacuum_affine_spatial_current import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_spatial_Hamiltonian_and_vertices",
    "two_momentum_response_and_finite_band",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["two_momentum_response_and_finite_band"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 25 and d["checked_scalar_entries"] == 272
    assert len(d["proof_checks"]) == 39
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 117
    assert "NOT_RENORMALIZED_SPATIAL_CONTINUUM_INVERSE" in d["status"]
