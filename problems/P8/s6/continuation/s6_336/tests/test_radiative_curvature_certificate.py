...
import pytest
from p8_vacuum_affine_radiative_curvature_matching import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_covariant_contact_and_calibrations",
    "whole_flat_matching_obstruction_and_conditional_bounds",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_flat_matching_obstruction_and_conditional_bounds"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_radiative_curvature_matching import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 19
    assert data["named_exact_check_count"] == len(audit.residuals()) == 314
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 719
    assert len(data["proof_checks"]) == len(audit.gates()) == 46
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 289
    assert "NOT_MATCHED_HARD_V_G_B_P8_CLOSURE" in data["status"]
