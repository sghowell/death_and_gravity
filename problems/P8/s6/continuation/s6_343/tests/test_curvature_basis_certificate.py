"""Whole twenty-field native equality and immutable curvature-basis contract."""

import copy
import json

import pytest
from p8_vacuum_affine_curvature_contact_basis import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_basis_and_selected_TT_poles",
    "whole_strict_matching_and_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_strict_matching_and_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_curvature_contact_basis import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 20
    assert data["named_exact_check_count"] == len(audit.residuals()) == 1435
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 1456
    assert len(data["proof_checks"]) == len(audit.gates()) == 41
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 606
