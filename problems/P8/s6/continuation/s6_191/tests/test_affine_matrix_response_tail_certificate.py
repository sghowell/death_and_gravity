...
import pytest
from p8_vacuum_affine_matrix_response_tail import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_mixed_jets_and_reference",
    "actual_C2_graph_covariance_and_weighted_tail",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["actual_C2_graph_covariance_and_weighted_tail"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 225 and d["checked_scalar_entries"] == 370
    assert len(d["proof_checks"]) == 39
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 113
    assert "NOT_COMPLETE_COVARIANT_STRESS_RESPONSE_INVERSE" in d["status"]
