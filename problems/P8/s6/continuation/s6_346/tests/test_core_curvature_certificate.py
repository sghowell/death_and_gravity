...
import pytest
from p8_vacuum_affine_core_curvature_coefficient import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_common_basis_conversion_and_exact_moments",
    "whole_core_sum_bounds_and_original_calibration",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_core_sum_bounds_and_original_calibration"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_core_curvature_coefficient import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 21
    assert data["named_exact_check_count"] == len(audit.residuals()) == 447
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 465
    assert len(data["proof_checks"]) == len(audit.gates()) == 47
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 795
