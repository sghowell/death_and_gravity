...
import pytest
from p8_vacuum_double_bubble_ms import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_MS_forests_and_full_scale_factors",
    "exact_quadratic_forward_term_and_bounds",
    "actual_double_bubble_MS_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["actual_MS_forests_and_full_scale_factors"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 65
    assert d["checked_scalar_entries"] == 65
    assert len(d["proof_checks"]) == 23
    assert d["controls"]["rejected_inputs"] == 94
