...
import pytest
from p8_vacuum_affine_curved_scalar_reference import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_curved_geometry_and_complete_original_local_Hessian",
    "ordered_curvature_coordinate_reference_inverse_and_physical_density",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["ordered_curvature_coordinate_reference_inverse_and_physical_density"]
    else:
...
def test_exact_counts_and_uncompleted_actual_quantum_boundary():
    ...
    assert (
        data["named_exact_check_count"] == 38 and data["checked_scalar_entries"] == 55
    )
    assert len(data["proof_checks"]) == 24
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 169
    assert "NOT_ACTUAL_FULL_CURVED_QUANTUM_INVERSE" in data["status"]
    assert (
        data["prior_sha256"][
