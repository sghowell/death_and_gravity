"""Whole twenty-field equality and frozen mixed-source contract."""

import copy
import json

import pytest
from p8_vacuum_affine_mixed_source_radiation import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_contractions_and_exact_D_kernel",
    "whole_complete_radiation_bounds_and_original_calibrations",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_complete_radiation_bounds_and_original_calibrations"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_mixed_source_radiation import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 22
    assert data["named_exact_check_count"] == len(audit.residuals()) == 331
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 331
    assert len(data["proof_checks"]) == len(audit.gates()) == 58
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 365
