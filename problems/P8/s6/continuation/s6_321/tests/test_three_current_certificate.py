...
import pytest
from p8_vacuum_affine_three_soft_current_subtraction import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_angular_coverage",
    "whole_three_current_and_one_block_subtraction",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_three_current_and_one_block_subtraction"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    from p8_vacuum_affine_three_soft_current_subtraction import audit, subtraction
    ...
    assert len(subtraction.data()["checks"]) == 1056
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 73
    assert "NOT_FULL_INCLUSIVE_V_G_B_P8_CLOSURE" in data["status"]
