...
import pytest
from p8_vacuum_fermion_insertion_ms import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_finite_MS_local_anchors",
    "fixed_source_and_forest_ownership",
    "actual_paired_row_local_enclosure",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["full_finite_MS_local_anchors"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 47
    assert d["checked_scalar_entries"] == 47
    assert len(d["proof_checks"]) == 23
    assert d["controls"]["rejected_inputs"] == 105
    assert "NOT_VACUUM_OTHER_FORESTS" in d["status"]
