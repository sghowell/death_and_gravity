...
import pytest
from p8_vacuum_affine_full_adm_vertices import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_full_ADM_constraint_and_complete_metric_vertices",
    "four_metric_contact_chain_and_all_momentum_form_bounds",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    bad = copy.deepcopy(d)
    if mutation == "missing":
        del bad["four_metric_contact_chain_and_all_momentum_form_bounds"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 39 and d["checked_scalar_entries"] == 697
    assert len(d["proof_checks"]) == 35
    assert len(d["controls"]) == 9 and d["controls"]["rejected_inputs"] == 136
    assert "NOT_FULL_NEW_SECTOR_CONTINUUM_MATCHING_REDUCED_INVERSE" in d["status"]
