...
import pytest
from p8_vacuum_affine_radiative_soft_state_transfer import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_uniform_state_continuity",
...
def test_missing_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_paired_soft_transfer"]
    else:
...
def test_counts_and_historical_scope():
    from p8_vacuum_affine_radiative_soft_state_transfer import audit

    data = verify.build_report()
    assert len(data["source_sha256"]) == 17
    assert data["named_exact_check_count"] == len(audit.residuals()) == 260
    assert data["checked_scalar_entries"] == audit.scalar_entry_count() == 383
    assert len(data["proof_checks"]) == len(audit.gates()) == 89
    assert len(data["controls"]) == 8 and data["controls"]["rejected_inputs"] == 83
