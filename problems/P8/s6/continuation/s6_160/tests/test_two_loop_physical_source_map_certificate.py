"""Read-only source/report checks and every-field mutation rejection."""

import copy
import json

import pytest
from p8_vacuum_two_loop_physical_source_map import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_generated_action_source_and_loop_support",
    "regulated_Ward_and_source_transport",
    "actual_physical_source_two_loop_enclosures",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["regulated_Ward_and_source_transport"]
    else:
        changed["ordinary_Psi_second_residue_bounded"] = True
    with pytest.raises(ValueError, match="differs"):
        verify.validate_report(changed, d)
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 81
    assert d["checked_scalar_entries"] == 81
    assert len(d["proof_checks"]) == 23
    assert d["controls"]["rejected_inputs"] == 101
    assert "NOT_ORDINARY_PSI_MS_GLOBAL_INVERSE_PHYSICAL_TRUNCATION" in d["status"]
