...
import pytest
from p8_vacuum_affine_uniform_radiation_soft_limit import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_paired_currents",
    "whole_uniform_amplitudes_and_soft_threshold",
...
def test_missing_and_extra_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_uniform_amplitudes_and_soft_threshold"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert len(data["proof_checks"]) == 56
