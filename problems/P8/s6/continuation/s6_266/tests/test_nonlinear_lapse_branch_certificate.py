"""Frozen full nonlinear auxiliary-domain and physical-chain contract."""

import copy
import json

import pytest
from p8_vacuum_affine_nonlinear_lapse_branch import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_full_source_nonlinear_domain_and_auxiliary_pivots",
    "whole_full_implicit_and_physical_composite_responses",
...
def test_missing_and_extra_report_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_full_source_nonlinear_domain_and_auxiliary_pivots"]
    else:
...
def test_exact_counts_and_unchanged_quantum_domain_boundary():
    ...
    assert len(data["proof_checks"]) == 45
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 291
    assert (
        "NOT_CANONICAL_GAUSSIAN_SUPPORT_INTERACTING_MEAN_QUANTUM_REGULATOR_CUTOFF_OR_ORIGINAL_V_G_B_P8"
        in data["status"]
    )
    assert (
        data["prior_sha256"][
            "S265_Gaussian_coefficient_boundary_and_S261_refutation_unchanged"
        ]
        is True
    )
