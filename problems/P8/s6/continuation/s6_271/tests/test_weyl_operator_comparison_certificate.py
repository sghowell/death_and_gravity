"""Immutable full finite Weyl operator ordering, positivity and unitary comparison contract."""

import copy
import json

import pytest
from p8_vacuum_affine_weyl_operator_comparison import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_full_source_and_explicit_Weyl_operator_bound",
    "whole_high_derivatives_and_evaluated_ordering_unitary_comparison",
...
def test_missing_and_extra_report_fields_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_full_source_and_explicit_Weyl_operator_bound"]
    else:
...
def test_exact_counts_and_original_matching_boundary():
    ...
    assert len(data["proof_checks"]) == 43
    assert len(data["controls"]) == 9 and data["controls"]["rejected_inputs"] == 235
    assert "NOT_ORIGINAL_PHYSICAL_MATCHING_CONTINUUM_LOOP_OR_V_G_B_P8" in data["status"]
    assert (
        data["prior_sha256"][
