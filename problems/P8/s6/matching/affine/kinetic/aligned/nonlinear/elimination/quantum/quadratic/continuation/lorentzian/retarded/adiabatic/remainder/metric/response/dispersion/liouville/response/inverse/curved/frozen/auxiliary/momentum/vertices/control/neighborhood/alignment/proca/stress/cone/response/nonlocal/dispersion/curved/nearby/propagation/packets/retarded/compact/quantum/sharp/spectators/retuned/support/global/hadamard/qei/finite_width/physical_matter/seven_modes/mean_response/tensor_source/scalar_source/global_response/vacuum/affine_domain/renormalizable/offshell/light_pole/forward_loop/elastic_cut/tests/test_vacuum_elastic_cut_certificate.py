...
import pytest
from p8_vacuum_elastic_cut import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_physical_tree_amplitude",
    "exact_angular_integral",
    "independent_cut_normalization",
    "actual_low_energy_cut_and_subtracted_margin",
    "exact_invariant_calibrations",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_report_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["actual_low_energy_cut_and_subtracted_margin"]
    else:
...
def test_exact_counts_and_original_scope():
    d = verify.build_report()
    assert d["claim"] == "P8-S6.114.POLYNOMIAL_VACUUM_ONE_LOOP_ELASTIC_CUT"
    assert d["named_exact_check_count"] == 33
    assert d["checked_scalar_entries"] == 33
