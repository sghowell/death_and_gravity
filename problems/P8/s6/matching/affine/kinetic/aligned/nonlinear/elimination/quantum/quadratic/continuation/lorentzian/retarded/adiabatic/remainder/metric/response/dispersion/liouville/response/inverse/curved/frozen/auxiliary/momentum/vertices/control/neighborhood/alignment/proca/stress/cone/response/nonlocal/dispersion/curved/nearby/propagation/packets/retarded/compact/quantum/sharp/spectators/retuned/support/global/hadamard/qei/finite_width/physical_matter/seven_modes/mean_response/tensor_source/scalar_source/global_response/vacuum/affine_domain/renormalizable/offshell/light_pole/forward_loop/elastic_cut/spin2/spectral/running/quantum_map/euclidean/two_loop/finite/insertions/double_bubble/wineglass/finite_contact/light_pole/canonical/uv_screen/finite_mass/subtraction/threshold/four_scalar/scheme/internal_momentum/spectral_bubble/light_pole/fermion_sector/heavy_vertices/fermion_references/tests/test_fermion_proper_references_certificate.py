...
import pytest
from p8_vacuum_fermion_proper_references import verify
...
KEYS = [
    ...
    "proof_checks",
    "fermion_inverse_and_dimensional_anchors",
    "proper_counterterms_and_field_conversion",
    "exact_reference_enclosures",
    "actual_common_reference_and_primitive_frontier",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["proper_counterterms_and_field_conversion"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 76
    assert d["checked_scalar_entries"] == 357
    assert len(d["proof_checks"]) == 30
    assert d["controls"]["rejected_inputs"] == 68
    assert "NOT_COMPLETE_TWO_LOOP_PRIMITIVES" in d["status"]
