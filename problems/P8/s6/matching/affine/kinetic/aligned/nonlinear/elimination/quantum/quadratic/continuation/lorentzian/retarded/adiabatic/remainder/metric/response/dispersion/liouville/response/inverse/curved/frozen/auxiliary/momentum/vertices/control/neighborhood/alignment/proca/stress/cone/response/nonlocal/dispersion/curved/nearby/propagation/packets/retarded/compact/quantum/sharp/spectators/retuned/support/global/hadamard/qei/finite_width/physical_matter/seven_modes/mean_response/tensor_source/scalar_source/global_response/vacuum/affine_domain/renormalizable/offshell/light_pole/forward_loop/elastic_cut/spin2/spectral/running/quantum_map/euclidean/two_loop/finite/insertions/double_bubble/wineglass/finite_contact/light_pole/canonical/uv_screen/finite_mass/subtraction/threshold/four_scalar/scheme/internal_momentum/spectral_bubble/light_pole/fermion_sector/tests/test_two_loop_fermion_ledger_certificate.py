...
import pytest
from p8_vacuum_two_loop_fermion_ledger import verify
...
KEYS = [
    ...
    "proof_checks",
    "loop_and_background_expansion",
    "gauge_color_flavor_ownership",
    "counterterm_and_finite_conversion_ledger",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["gauge_color_flavor_ownership"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 141
    assert d["checked_scalar_entries"] == 141
    assert len(d["proof_checks"]) == 28
    assert d["controls"]["rejected_inputs"] == 39
    assert "NOT_EVALUATED_TWO_LOOP_ERROR" in d["status"]
