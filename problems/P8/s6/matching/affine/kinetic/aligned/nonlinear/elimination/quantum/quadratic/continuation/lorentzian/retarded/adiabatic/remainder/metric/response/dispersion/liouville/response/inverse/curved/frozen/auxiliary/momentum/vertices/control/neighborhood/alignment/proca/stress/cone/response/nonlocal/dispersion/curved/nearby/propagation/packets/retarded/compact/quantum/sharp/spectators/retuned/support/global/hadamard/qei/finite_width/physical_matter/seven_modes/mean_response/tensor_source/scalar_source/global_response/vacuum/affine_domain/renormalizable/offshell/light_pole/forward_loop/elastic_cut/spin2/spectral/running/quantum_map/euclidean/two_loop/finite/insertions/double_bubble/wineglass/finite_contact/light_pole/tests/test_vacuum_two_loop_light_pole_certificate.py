...
import pytest
from p8_vacuum_two_loop_light_pole import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_two_point_graph_and_forest_inventory",
    "independent_denominators_and_integrated_sectors",
    "actual_subtractions_and_complex_analytic_bounds",
    "actual_two_loop_light_pole_and_error_budget",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_report_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["actual_two_loop_light_pole_and_error_budget"]
    else:
...
def test_exact_counts_and_original_scope():
    d = verify.build_report()
    assert d["named_exact_check_count"] == d["checked_scalar_entries"] == 743
    assert len(d["proof_checks"]) == 32
    assert len(d["source_sha256"]) == 21
    assert d["controls"]["rejected_inputs"] == 33
    assert "NOT_COMPLETE_TWO_LOOP_FOUR_POINT_SOURCE_MATCHING" in d["status"]
