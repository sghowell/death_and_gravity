...
import pytest
from p8_vacuum_gauge_light_cut_subtraction import verify
...
KEYS = [
    ...
    "proof_checks",
    "complex_finite_mass_transition_domain",
    "finite_cut_and_boundary_germs",
    "holomorphic_coefficient_error_bound",
    "same_candidate_subtraction_coefficient",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["finite_cut_and_boundary_germs"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == d["checked_scalar_entries"] == 80
    assert len(d["proof_checks"]) == 29
    assert d["controls"]["rejected_inputs"] == 24
    assert "NOT_FULL_MATCHED_AMPLITUDE" in d["status"]
