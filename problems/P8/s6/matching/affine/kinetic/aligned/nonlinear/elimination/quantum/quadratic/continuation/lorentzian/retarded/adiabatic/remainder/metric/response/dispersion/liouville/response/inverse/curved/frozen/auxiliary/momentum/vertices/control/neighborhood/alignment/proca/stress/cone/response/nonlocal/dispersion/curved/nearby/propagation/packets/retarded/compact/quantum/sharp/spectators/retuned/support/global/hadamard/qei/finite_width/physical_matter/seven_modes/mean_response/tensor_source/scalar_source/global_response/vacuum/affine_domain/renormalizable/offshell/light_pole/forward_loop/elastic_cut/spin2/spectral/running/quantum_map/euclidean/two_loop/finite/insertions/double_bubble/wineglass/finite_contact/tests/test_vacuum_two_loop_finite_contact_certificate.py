...
import pytest
from p8_vacuum_two_loop_finite_contact import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_Hessian_and_fixed_quartic_contact_condition",
    "independent_anchored_finite_contact_integrals",
    "complete_insertion_and_same_local_reference",
    "actual_fixed_contact_and_known_error_budget",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_report_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["actual_fixed_contact_and_known_error_budget"]
    else:
...
def test_exact_counts_and_original_scope():
    d = verify.build_report()
    assert d["named_exact_check_count"] == d["checked_scalar_entries"] == 40
    assert len(d["proof_checks"]) == 34
    assert len(d["source_sha256"]) == 18
    assert d["controls"]["rejected_inputs"] == 14
    assert "NOT_COMPLETE_TWO_LOOP_POLE_LSZ_SOURCE_MATCHING" in d["status"]
