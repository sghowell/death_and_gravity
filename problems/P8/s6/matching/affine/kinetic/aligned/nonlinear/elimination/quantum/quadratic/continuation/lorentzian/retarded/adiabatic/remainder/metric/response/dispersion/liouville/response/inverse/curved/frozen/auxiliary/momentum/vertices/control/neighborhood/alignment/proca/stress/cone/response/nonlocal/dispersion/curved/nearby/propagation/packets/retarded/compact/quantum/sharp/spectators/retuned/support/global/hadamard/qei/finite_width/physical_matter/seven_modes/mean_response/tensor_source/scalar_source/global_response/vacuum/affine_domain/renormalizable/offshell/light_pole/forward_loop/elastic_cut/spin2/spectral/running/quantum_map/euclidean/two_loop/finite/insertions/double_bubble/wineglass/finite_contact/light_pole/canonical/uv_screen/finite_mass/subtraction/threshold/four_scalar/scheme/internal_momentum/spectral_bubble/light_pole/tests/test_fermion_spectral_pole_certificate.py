...
import pytest
from p8_vacuum_fermion_spectral_pole import verify
...
KEYS = [
    ...
    "proof_checks",
    "literal_quadratic_covariance_insertion",
    "spectral_outer_kernel_and_enclosure",
    "selected_functional_order_ledger",
    "actual_selected_pole_bounds",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["spectral_outer_kernel_and_enclosure"]
    else:
