...
import pytest
from p8_vacuum_full_one_loop_matching import verify
...
KEYS = [
    ...
    "proof_checks",
    "full_local_reference_ledger",
    "bare_coupling_and_scheme_conversion",
    "complete_one_loop_amplitude",
    "actual_pole_and_coefficient_bounds",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["bare_coupling_and_scheme_conversion"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 57
    assert d["checked_scalar_entries"] == 57
    assert len(d["proof_checks"]) == 36
    assert d["controls"]["rejected_inputs"] == 61
    assert "NOT_HIGHER_LOOP_ERROR" in d["status"]
