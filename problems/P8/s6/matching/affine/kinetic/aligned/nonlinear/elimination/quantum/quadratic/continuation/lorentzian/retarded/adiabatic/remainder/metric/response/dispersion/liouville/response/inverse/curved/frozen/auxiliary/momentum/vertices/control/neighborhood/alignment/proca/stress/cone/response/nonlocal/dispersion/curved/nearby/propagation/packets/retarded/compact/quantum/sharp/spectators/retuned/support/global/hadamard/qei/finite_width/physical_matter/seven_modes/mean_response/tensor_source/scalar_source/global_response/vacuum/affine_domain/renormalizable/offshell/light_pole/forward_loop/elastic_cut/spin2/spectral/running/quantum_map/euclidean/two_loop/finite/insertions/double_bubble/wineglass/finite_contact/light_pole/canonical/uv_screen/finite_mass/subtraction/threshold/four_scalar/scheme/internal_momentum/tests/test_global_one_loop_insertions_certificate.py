...
import pytest
from p8_vacuum_global_one_loop_insertions import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_kernel_halfplane_insertion",
    "global_spacelike_decay_envelope",
    "selected_all_momentum_routing_and_window",
    "actual_reference_window_and_insertion_bounds",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["global_spacelike_decay_envelope"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["controls"]["rejected_inputs"] == 196
    assert "NOT_OUTER_TWO_LOOP_HIGHER_LOOP_ERROR" in d["status"]
