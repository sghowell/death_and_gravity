"""New constant-mass tangent: re-evaluate the generic comparison algebra.

The unchanged S6.68 polynomial algorithm is copied into this new source
namespace so all physical vertices, varied references and readouts are
computed for the new action. No parent function or result is replaced.
"""
...
def reference(sector):
    return _reference(source.kind(sector))


@cache
def _reference(sector):
    data = source.data(sector)
    ...
    return {"S": S, "dS": dS, "R": R, "dR": dR, "T": T, "dT": dT,
            ...
            "tenth_source_derivative_coefficients": {name: sp.factor(value) for name, value in highest.items()},
