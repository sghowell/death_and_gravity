...
import pytest
from p8_vacuum_fermion_spectral_bubble import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_spectral_insertion",
    "positive_measure_and_outer_subtraction",
    "six_placement_reference_ownership",
    "actual_family_coefficient_bounds",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["positive_measure_and_outer_subtraction"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["controls"]["rejected_inputs"] == 54
    assert "NOT_COMPLETE_TWO_LOOP_HIGHER_LOOP_ERROR" in d["status"]
