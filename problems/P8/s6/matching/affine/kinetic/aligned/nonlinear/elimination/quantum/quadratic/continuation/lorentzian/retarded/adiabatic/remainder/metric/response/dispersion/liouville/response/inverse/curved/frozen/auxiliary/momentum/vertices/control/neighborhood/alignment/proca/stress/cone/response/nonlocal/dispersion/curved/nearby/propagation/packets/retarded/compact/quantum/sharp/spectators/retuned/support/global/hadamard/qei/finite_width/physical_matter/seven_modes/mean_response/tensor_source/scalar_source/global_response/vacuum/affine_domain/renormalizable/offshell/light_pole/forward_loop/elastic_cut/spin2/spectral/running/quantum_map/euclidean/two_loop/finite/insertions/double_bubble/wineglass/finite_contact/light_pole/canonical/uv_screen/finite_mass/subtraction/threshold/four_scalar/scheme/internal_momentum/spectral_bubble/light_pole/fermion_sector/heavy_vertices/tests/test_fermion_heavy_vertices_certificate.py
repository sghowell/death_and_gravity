...
import pytest
from p8_vacuum_fermion_heavy_vertices import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_vertex_ownership",
    "mass_replacement_and_spectral_bounds",
    "local_parent_subtraction",
    "actual_enclosure_and_remaining_frontier",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["mass_replacement_and_spectral_bounds"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 60
    assert d["checked_scalar_entries"] == 60
    assert len(d["proof_checks"]) == 29
    assert d["controls"]["rejected_inputs"] == 106
    assert "NOT_COMPLETE_TWO_LOOP_FINITE_MS_MATCHING" in d["status"]
