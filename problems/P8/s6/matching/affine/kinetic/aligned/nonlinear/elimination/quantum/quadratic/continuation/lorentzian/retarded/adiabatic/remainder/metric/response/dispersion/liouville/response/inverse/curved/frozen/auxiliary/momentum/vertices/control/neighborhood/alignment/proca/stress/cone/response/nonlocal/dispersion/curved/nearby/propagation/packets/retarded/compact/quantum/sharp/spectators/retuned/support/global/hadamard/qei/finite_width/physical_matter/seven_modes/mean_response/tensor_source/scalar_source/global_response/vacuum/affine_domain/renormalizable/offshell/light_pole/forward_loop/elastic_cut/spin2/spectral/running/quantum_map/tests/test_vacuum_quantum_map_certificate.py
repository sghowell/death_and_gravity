"""Read-only complete ancestry and every report-field mutation."""

import copy
import json

import pytest
from p8_vacuum_quantum_map import verify
...
KEYS = [
    ...
    "proof_checks",
    "actual_derivative_map_Wick_mixing",
    "all_generated_interactions_and_counterterms",
    "source_identity_controls_and_LSZ",
    "actual_composite_and_propagator_bounds",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_report_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["actual_composite_and_propagator_bounds"]
    else:
...
def test_exact_counts_and_original_scope():
    d = verify.build_report()
    assert d["claim"] == "P8-S6.118.POLYNOMIAL_VACUUM_ONE_LOOP_QUANTUM_FIELD_MAP"
    assert d["named_exact_check_count"] == d["checked_scalar_entries"] == 55
    assert len(d["proof_checks"]) == 32
    assert d["controls"]["rejected_inputs"] == 38
