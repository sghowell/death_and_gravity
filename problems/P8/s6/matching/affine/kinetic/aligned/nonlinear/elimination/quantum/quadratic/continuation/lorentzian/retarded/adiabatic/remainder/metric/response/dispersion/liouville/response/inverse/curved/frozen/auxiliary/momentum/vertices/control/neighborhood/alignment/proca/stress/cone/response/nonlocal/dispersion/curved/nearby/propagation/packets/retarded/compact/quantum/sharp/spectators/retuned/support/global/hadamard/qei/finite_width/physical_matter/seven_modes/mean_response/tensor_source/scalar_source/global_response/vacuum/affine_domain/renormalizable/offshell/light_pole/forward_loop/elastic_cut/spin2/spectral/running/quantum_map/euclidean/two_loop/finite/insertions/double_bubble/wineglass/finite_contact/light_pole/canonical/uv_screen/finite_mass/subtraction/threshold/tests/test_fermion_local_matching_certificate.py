...
import pytest
from p8_vacuum_fermion_local_matching import verify
...
KEYS = [
    ...
    "proof_checks",
    "all_flavor_fermion_potential",
    "two_point_and_cancellation_free_anchors",
    "explicit_local_reference_and_field_map",
    "same_candidate_selected_functional_bounds",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["two_point_and_cancellation_free_anchors"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == d["checked_scalar_entries"] == 67
    assert len(d["proof_checks"]) == 32
    assert d["controls"]["rejected_inputs"] == 30
    assert "NOT_FULL_NEW_MODEL_QUANTUM" in d["status"]
