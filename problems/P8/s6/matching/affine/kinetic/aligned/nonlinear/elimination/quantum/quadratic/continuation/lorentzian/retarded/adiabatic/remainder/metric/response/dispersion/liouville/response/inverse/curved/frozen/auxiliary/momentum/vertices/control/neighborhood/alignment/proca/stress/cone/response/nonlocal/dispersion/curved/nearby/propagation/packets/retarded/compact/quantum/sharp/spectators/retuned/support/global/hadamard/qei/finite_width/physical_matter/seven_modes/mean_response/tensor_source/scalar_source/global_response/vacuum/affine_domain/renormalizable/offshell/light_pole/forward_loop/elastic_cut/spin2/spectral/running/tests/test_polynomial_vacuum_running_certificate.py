"""Complete read-only ancestry and all report-field mutations."""

import copy
import json

import pytest
from p8_polynomial_vacuum_running import verify
...
KEYS = [
    ...
    "proof_checks",
    "literal_two_field_beta_tensors",
    "anchored_reference_change_and_full_channel_compensation",
    "closed_one_loop_flow_and_positive_margin",
    "actual_uniform_and_pointwise_reference_enclosures",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_report_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["actual_uniform_and_pointwise_reference_enclosures"]
    else:
...
def test_exact_counts_and_original_scope():
    d = verify.build_report()
    assert d["claim"] == "P8-S6.117.POLYNOMIAL_VACUUM_ONE_LOOP_REFERENCE_FLOW"
    assert d["named_exact_check_count"] == d["checked_scalar_entries"] == 61
