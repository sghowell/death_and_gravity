...
import pytest
from p8_vacuum_two_loop_insertions import verify
...
KEYS = [
    ...
    "proof_checks",
    "grouped_topology_and_fixed_local_subtractions",
    "actual_complex_kernel_and_constant_remainder_split",
    "all_three_channel_complex_momentum_routings",
    "convergent_radial_integral_and_actual_partial_bound",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_report_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["convergent_radial_integral_and_actual_partial_bound"]
    else:
...
def test_exact_counts_and_original_scope():
    d = verify.build_report()
    assert d["named_exact_check_count"] == d["checked_scalar_entries"] == 51
    assert len(d["proof_checks"]) == 34
    assert len(d["source_sha256"]) == 18
    assert d["controls"]["rejected_inputs"] == 14
    assert "NOT_REMAINING_FORTY_REFINEMENTS_COMPLETE_TWO_LOOP_POLE_LSZ" in d["status"]
