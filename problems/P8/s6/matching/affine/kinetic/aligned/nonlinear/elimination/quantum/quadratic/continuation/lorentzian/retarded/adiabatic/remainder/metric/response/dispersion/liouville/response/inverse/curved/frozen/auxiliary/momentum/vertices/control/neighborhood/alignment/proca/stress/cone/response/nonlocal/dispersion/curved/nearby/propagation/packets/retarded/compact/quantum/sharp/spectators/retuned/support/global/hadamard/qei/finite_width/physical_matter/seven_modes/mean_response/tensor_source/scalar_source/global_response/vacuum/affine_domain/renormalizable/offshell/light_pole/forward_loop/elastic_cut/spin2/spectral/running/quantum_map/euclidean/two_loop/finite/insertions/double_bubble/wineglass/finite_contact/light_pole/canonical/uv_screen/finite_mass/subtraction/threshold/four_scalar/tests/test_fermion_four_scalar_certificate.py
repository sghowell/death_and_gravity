...
import pytest
from p8_vacuum_fermion_four_scalar import verify
...
KEYS = [
    ...
    "proof_checks",
    "complete_low_degree_matching",
    "complex_routes_and_all_degree_remainder",
    "explicit_field_and_potential_dictionary",
    "same_boundary_selected_coefficient_band",
    "controls",
    "verdict",
    "not_established",
...
def test_missing_or_extra_field_rejected(mutation):
    ...
    changed = copy.deepcopy(d)
    if mutation == "missing":
        del changed["complex_routes_and_all_degree_remainder"]
    else:
...
def test_exact_counts_and_scope():
    ...
    assert d["named_exact_check_count"] == 127
    assert d["checked_scalar_entries"] == 254
    assert len(d["proof_checks"]) == 29
    assert d["controls"]["rejected_inputs"] == 43
    assert "NOT_FULL_NEW_MODEL_SCALAR" in d["status"]
