"""Read-only ancestry, manifest and report-mutation controls."""
import copy
import json

import pytest
from p8_aligned_onshell import verify
...
@pytest.mark.parametrize("key", ("claim", "status", "source_sha256", "prior_sha256", "exact_residuals",
                                  "proof_checks", "literal_action", "physical_phase", "geometric_readout",
                                  "initial_preparation", "continuous_bounds", "domain_and_scaling",
                                  "source_and_response_transfer", "example", "controls", "verdict",
...
def test_control_count_and_admissibility_boundary():
    assert verify.controls()["rejected_inputs"] == 27
    assert verify.controls()["on_shell_means_linear_light_not_full_nonlinear_parent"] is True
