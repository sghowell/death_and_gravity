"""Entire immutable twenty-field report equality, not selected-field approval."""

import copy
import json

import pytest
from p8_vacuum_affine_massive_graviton_pair_cut import verify
...
KEYS = [
    ...
    "proof_checks",
    "whole_original_source_and_literal_graviton_pair_tree",
    "whole_physical_angular_spectral_and_window_cut",
...
def test_missing_and_extra_fields_are_rejected(mutation):
    ...
    bad = copy.deepcopy(data)
    if mutation == "missing":
        del bad["whole_physical_angular_spectral_and_window_cut"]
    else:
...
def test_exact_counts_and_historical_scope():
    ...
    assert (
        data["prior_sha256"]["S276_S277_paths_not_reidentified_as_the_original_state"]
        is True
    )
    assert (
        data["prior_sha256"]["S278_M1_cut_and_conditional_soft_dictionary_unchanged"]
        is True
    )
    assert (
